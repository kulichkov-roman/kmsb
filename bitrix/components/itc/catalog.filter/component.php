...
if($obCache->StartDataCache($arParams["CACHE_TIME"], $this->GetCacheID(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())), "/".SITE_ID.$this->GetRelativePath()))
{
    ...
    "=UF_HIDDEN" => "",//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		{
			$arResult["arrSection"][$arSection["ID"]] = str_repeat(" . ", $arSection["DEPTH_LEVEL"]-1).$arSection["NAME"];
		}
	}
...
foreach($arResult["arrProp"] as $prop_id=>$arProp)
{
	...
	switch ($arProp["PROPERTY_TYPE"])
	{
		...
		case "N":
			...
			$res .= 'От&nbsp;<input type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialchars($value).'" />&nbsp;&nbsp;&nbsp;До&nbsp;';
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$arResult["arrInputNames"][$FILTER_NAME."_cf"]=true;
$name = $FILTER_NAME."_cf[QUANTITY][LEFT]";
$value = $arrCFV["QUANTITY"]["LEFT"];
$checked = "";
if (strlen($value)>0)
{
 $checked = "checked";
 if(CModule::IncludeModule("catalog"))
  ${$FILTER_NAME}[">=CATALOG_QUANTITY"] = 1;
 else
  ${$FILTER_NAME}[">=PROPERTY_QUANTITY"] = 1;
}


$arResult["ITEMS"][] = array("NAME" => "", "INPUT" => '<input type="checkbox" class="catalog-search-check" '.$checked.' name="'.$name.'" value="1" />&nbsp;В наличии');
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
