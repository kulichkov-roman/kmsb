...
if(!empty($productIds)) {
	$rsProducts = CIBlockElement::GetList(array('ID' => 'ASC'), array('IBLOCK_ID' => CATALOG_IBLOCK_ID_KS, 'ID' => $productIds), false, false, array('ID', 'XML_ID', 'DETAIL_PAGE_URL', 'DETAIL_PICTURE', 'PROPERTY_KOMPLEKTUYUSHCHIE', 'PROPERTY_NAIMENOVANIE_DLYA_SAYTA_POLNOE', 'PROPERTY_NAIMENOVANIE_DLYA_SAYTA_KRATKOE'));
...
if(!empty($xmlIds)) {
	$rsProducts = CIBlockElement::GetList(array('ID' => 'ASC'), array('IBLOCK_ID' => CATALOG_IBLOCK_ID_KS, 'XML_ID' => $xmlIds), false, false, array('ID', 'NAME', 'XML_ID', 'DETAIL_PAGE_URL', 'DETAIL_PICTURE', 'PROPERTY_NAIMENOVANIE_DLYA_SAYTA_POLNOE', 'PROPERTY_NAIMENOVANIE_DLYA_SAYTA_KRATKOE'));
	while($arProduct = $rsProducts->GetNext()) {
		//$arResult['PRODUCTS'][$arProduct['ID']] = $arProduct;
	...
	if(empty($arResult['ERRORS'])) {
		...
		foreach($arResult['ITEMS'] as $id => $arItem) {
			...
			if(!empty($arItem['OPTIONS'])) {
				...
				$productsTable .= '</table></td></tr>';
			}
			
			$pNum++;
		}
		$productsTable .= '<tr><td colspan="3">Итого</td><td>' . getPrintPrice($arResult['ORDER_INFO']['SUM']) . '</td></tr>';
		$productsTable .= '</table>';
