...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	else // if there;s no any errors - create user
	{
		...
		if ($bOk)
		{
			...
			$arUserFilter = array("ID" => $ID);

			$rsUser = CUser::GetList($by, $order, $arUserFilter, array("SELECT" => array("UF_*")));
			if($arUser = $rsUser -> GetNext()){
				$arProfileProps = array(
					27 => $arUser['NAME'],
					20 => $arUser['WORK_COMPANY'],
					30 => $arUser['WORK_NOTES'],
					24 => $arUser['WORK_PHONE'],
					25 => $arUser['PERSONAL_PHONE'],
					26 => $arUser['WORK_FAX'],
					22 => $arUser['UF_INN'],
					23 => $arUser['UF_KPP'],
					21 => $arUser['WORK_STREET'],
					29 => $arUser['EMAIL']
				);
				$userTypeId = 2; //физ.лицо

				CSaleOrderUserProps::DoSaveUserProfile($ID, false, $arProfileProps[27], $userTypeId, $arProfileProps, $arProfileErrors);
			}
		}
		...
		if (intval($ID) > 0)
		{
			...
			$rsUsers = CUser::GetList($order, $tmp, array("ID" => $ID), array("SELECT" => array("UF_*")));
			...
			if($arFiles){
				SendAttache("NEW_USER", SITE_ID, $arEventFields, $arFiles);
			} else {
