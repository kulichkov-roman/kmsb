...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])) && empty($_REQUEST["NAME_USER_FALSE"]))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			$_SESSION["CONTACTS_ADDED_MESSAGE"] = $arParams["USER_MESSAGE_".$sAction];

			$elementId = $arParams["ID"];
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = $_SESSION["CONTACTS_ADDED_MESSAGE"]/*htmlspecialcharsex($_REQUEST["strIMessage"])*/;
		unset($_SESSION["CONTACTS_ADDED_MESSAGE"]);
