...
if (count($arResult['IDS']) > 0 && CModule::IncludeModule('sale'))
{
 ...
 if($USER->IsAuthorized())
 {
  ...
  $arFilter = Array("IBLOCK_ID"=>DISCOUNT_IBLOCK_ID, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "PROPERTY_USER"=>$userID, "PROPERTY_SECTION"=>$arResult["ID"]);
    ...
    foreach($arItem["PRICES"] as $code=>$arPrice)
    {
     if($arPrice["CAN_ACCESS"])
     {
      ...
      if(floatVal($arPrice["VALUE"] - $arPrice["VALUE"]/100*$discount) < $price)
       echo "showPrice('price-".$arItem["ID"]."', '<span class=\"catalog-item-price\">".number_format(floatVal($arPrice["VALUE"] - $arPrice["VALUE"]/100*$discount), 0, ".", " ")." руб</span> <s>".$arPrice["PRINT_VALUE"]."</s>');\r\n";
     }
    }
   }
   echo '})</script>';
