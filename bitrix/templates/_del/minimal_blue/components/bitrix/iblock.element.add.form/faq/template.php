...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>#add_question" method="post" enctype="multipart/form-data">
 ...
 <?
  ...
  $new_list[] = "PREVIEW_TEXT";
  $arResult["PROPERTY_LIST"] = $new_list;
 ?>
        ...
        <label class="field-title"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?></label>
						  <div class="form-input"><textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea></div>
        </div>
								<?
								}
							break;
        ...
        <label class="field-title"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?></label>
								<div class="form-input"><input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /></div><?
   ...
   <?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
    <div class="field field-captcha">
		  <label class="field-title"><?=GetMessage("IBLOCK_FORM_CAPTCHA_TITLE")?></label>
		  <div class="form-input">
			...
			<?endif?>
		<?endif?>
  <div class="field field-button"><input type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" /></div>
 </div>
 </div>
</form>
</div>