...
{
 echo '<div class="errortext">По вашему запросу ничего не найдено.</div>';
	return;
}
?>
...
?>
				<div class="catalog-item-price" id="price-<?=$arElement["ID"]?>">
		...
		<?if ($arElement['CAN_BUY'] && $arElement['CATALOG_QUANTITY']):?>
			<a href="<?/*echo $arElement["ADD_URL"]*/?>/catalog/in_basket.php?id=<?=$arElement["ID"]?>" class="catalog-item-buy<?/*catalog-item-in-the-cart*/?>" rel="nofollow"  onclick="return addToCart(this, 'catalog_list_image_<?=$arElement['ID']?>', 'list', '<?=GetMessage("CATALOG_IN_CART")?>');" id="catalog_add2cart_link_<?=$arElement['ID']?>"><?echo GetMessage("CATALOG_ADD")?></a>
