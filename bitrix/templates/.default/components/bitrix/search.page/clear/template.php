...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):?>
			<div class="search-item">
        		...
        		<div class="search-announce">
        		  <h4><a href="<?echo $arItem["URL"]?>"><?echo getFormatSearchTitle(htmlentities($_GET["q"], ENT_QUOTES, "UTF-8"), $arItem["PROPERTY_NAIMENOVANIE_DLYA_SAYTA_POLNOE_VALUE"])?></a></h4>
  						<?if($arItem["PROPERTY_CML2_ARTICLE_VALUE"] <> ""){?>
  							<div><i>(артикул <?=getFormatSearchTitle(htmlentities($_GET["q"], ENT_QUOTES, "UTF-8"), $arItem["PROPERTY_CML2_ARTICLE_VALUE"])?>)</i></div>
  						<?}?>
  						<div class="search-preview"><?echo getDetilTextWithOutTable(truncateStr($arItem["DETAIL_TEXT"], 125, '', 'html','fp'))?></div>
        		</div>
				<?if(
		...
		<?//if($arParams["SHOW_ORDER_BY"] != "N"):?>
			...
			<?if($_GET["how"] == "nd"){?>
				<a href="<?=$arResult["URL"]?>&amp;how=na">По алфавиту от А до Я</a>&nbsp;<b>По алфавиту от Я до А</b>
			<?} elseif($_GET["how"] == "na"){?>
				<b>По алфавиту от А до Я</b>&nbsp;<a href="<?=$arResult["URL"]?>&amp;how=nd">По алфавиту от Я до А</a>
			<?} else {?>
				<b>По алфавиту от А до Я</b>&nbsp;<a href="<?=$arResult["URL"]?>&amp;how=nd">По алфавиту от Я до А</a>
			<?}?>
			</div>
